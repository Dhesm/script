...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 3366):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="3366"
...
Set_config_password(){
	...
	stty erase '^H' && read -p "(默认: Fatboy):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="Fatboy"
...
————————————
  ${Green_font_prefix}2.${Font_color_suffix} 一键封禁 BT/PT/SPAM (iptables)
  ${Green_font_prefix}3.${Font_color_suffix} 一键解封 BT/PT/SPAM (iptables)
